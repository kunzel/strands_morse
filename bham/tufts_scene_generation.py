...
robot.translate(x=0, y=-6.50, z=1.5)
#robot.translate(x=-1.35, y=0.56, z=7.5)
#robot.translate(x=-5.6, y=7, z=2.1)
robot.rotate(z=1.57)
...
pointer = PassiveObject('strands_sim/robots/tufts_objects.blend','pointer')
pointer.properties(Object = True, Type = 'Pointer')
pointer.translate(x=0,y=0,z=1.0)
...
for i in range(5):
###
   book[i] = PassiveObject('strands_sim/robots/tufts_objects.blend','book')
   book[i].properties(Object = True, Type = 'Book')
   book[i].translate(x=0,y=0,z=1.0)
   
   bottle[i] = PassiveObject('strands_sim/robots/tufts_objects.blend','bottle')
   bottle[i].properties(Object = True, Type = 'Bottle')
   bottle[i].translate(x=0,y=0,z=1.0)

   cup[i] = PassiveObject('strands_sim/robots/tufts_objects.blend','cup')
   cup[i].properties(Object = True, Type = 'Cup')
   cup[i].translate(0,0,1.0)

   keyboard[i] = PassiveObject('strands_sim/robots/tufts_objects.blend','keyboard')
   keyboard[i].properties(Object = True, Type = 'Keyboard')
   keyboard[i].translate(x=0,y=0,z=1.0)

   laptop[i] = PassiveObject('strands_sim/robots/tufts_objects.blend','laptop')
   laptop[i].properties(Object = True, Type = 'Laptop')
   laptop[i].translate(x=0,y=0,z=1.0)
   
   lamp[i] = PassiveObject('strands_sim/robots/tufts_objects.blend','lamp')
   lamp[i].properties(Object = True, Type = 'Lamp')
   lamp[i].translate(0, 0,1.0)

   monitor[i] = PassiveObject('strands_sim/robots/tufts_objects.blend','monitor')
...
env.set_camera_location([-5.6, 7, 2.1])
env.set_camera_rotation([0.8, 0.0, 1.57])
