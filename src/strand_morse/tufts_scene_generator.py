    ...
    def place_objects(self, no):
        ...
        for c in self.children:
            ...
            while i < MAX_NUM_OF_SAMPLES:
                x = x_mu #random.gauss(x_mu,self.x_sigma)
                y = y_mu #random.gauss(y_mu,self.y_sigma)
...
#def main(argv=None):
    #import bpy
    #bpy.data.objects['Compass'].hide_render = True
    ...
    try:
        ...
        print("Press 'Enter' to start scene generation (MORSE window should be active to capture images)")
        input()
        ...
        while i < num_of_scenes:
            ...
            if  not (('monitor' in objs and 'keyboard' in objs and not 'laptop' in objs) or
                     (not 'monitor' in objs and not 'keyboard' in objs and 'laptop' in objs)):
                continue
                    ...
                    with pymorse.Morse() as morse:
                        ...
                        for o in objs:
                            ...
                            if objs[o] in ['Monitor']:
                                node[o].set_yaw_range([0.0,0.0])
                            elif objs[o] in ['Keyboard','Laptop']:
                                node[o].set_yaw_range([0.0,0.0])
                            else:
